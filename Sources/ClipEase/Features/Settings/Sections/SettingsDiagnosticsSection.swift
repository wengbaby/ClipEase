...
struct SettingsDiagnosticsSection: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            SettingsSection(title: L("性能采样"), subtitle: diagnosticsSummaryText) {
                VStack(alignment: .leading, spacing: 12) {
                    Toggle(L("启用性能采样和日志记录"), isOn: $diagnostics.isEnabled)
                        .toggleStyle(.switch)

                    HStack(spacing: 10) {
                        historyButton(L("打开诊断目录"), minWidth: 104, action: onOpenLogsDirectory)
                        historyButton(L("清理诊断日志"), minWidth: 108, action: onCleanupLogs)
                    }

                    HStack(spacing: 16) {
                        Stepper(
                            L("保留 \(diagnostics.retentionDays) 天"),
                        ...
                        Stepper(
                            L("最多 \(diagnostics.maxLogSizeMB) MB"),
            ...
            SettingsSection(title: L("实时概览"), subtitle: L("最近 \(diagnostics.recentEvents.count) 条采样事件")) {
                VStack(alignment: .leading, spacing: 12) {
                    resourceOverview

...
- 完成记录：
  ...
  - 2026-06-07 阶段 3-B SQLiteRowMapper 拆分：
    ...
    - 本地提交：`bd97b40 refactor: extract sqlite row mapper`。
    - 阶段 3-C 入口条件：阶段 3-B 本地提交完成后，按顺序只允许进入 `SQLiteItemDAO` 拆分；第一轮只迁移 item 读取/写入 SQL，不改变 SQL 语义、排序、schema 或事务边界。
  - 2026-06-07 阶段 3-C SQLiteItemDAO 拆分：
    - 新增 `Sources/ClipEase/Core/Storage/SQLite/SQLiteItemDAO.swift`。
    - 新增 `Tests/ClipEaseTests/SQLiteItemDAOTests.swift`。
    - `SQLiteClipboardStore.swift` 不再直接维护 item 主表读取、按 ID 顺序读取、item 附件读取、file reference 读取、OCR 读取、item 主表写入、附件写入、file reference 写入、OCR 写入和 item 删除 SQL。
  ...
  - 2026-06-07 阶段 3-D SQLiteGroupDAO 拆分：
    ...
    - 本地提交：`3d7d246 refactor: extract sqlite group dao`。
    - 阶段 3-E 入口条件：阶段 3-D 本地提交完成后，按顺序只允许进入 `SQLiteSearchIndexDAO` 拆分；第一轮只迁移 FTS 写入、删除、准备索引和搜索 ID 查询，不改变搜索排序、FTS 表结构或分页语义。
  - 2026-06-07 阶段 3-E SQLiteSearchIndexDAO 拆分：
    - 新增 `Sources/ClipEase/Core/Storage/SQLite/SQLiteSearchIndexDAO.swift`。
    - 新增 `Tests/ClipEaseTests/SQLiteSearchIndexDAOTests.swift`。
    - `SQLiteClipboardStore.swift` 不再直接维护 FTS 搜索 ID 查询、FTS 写入、FTS 删除和搜索索引准备逻辑。

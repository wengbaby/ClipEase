...
- 历史窗口以可成为键盘窗口的非激活面板显示，不主动激活轻贴，避免把前台 App 从原编辑器切走。
- 历史窗口显示期间安装键盘事件路由，接管左右键、回车、空格、Esc、`Command + 1-9`、`Command + F/C/P`；关闭窗口或执行自动粘贴前立即卸载。
- 键盘事件路由依赖辅助功能权限；如系统拒绝事件监听，历史窗口仍可鼠标操作，但非激活模式下键盘操作可能不可用。
- 搜索、预览和卡片键盘操作都在非激活面板内完成；双击或回车粘贴时立即隐藏历史窗口并发送系统级 `Command + V`，让内容落到呼出轻贴前的 App 光标位置。
- 搜索结果变化时自动选中第一条结果；空结果时不允许回车粘贴，并关闭当前预览；Esc 在搜索展开时先退出搜索，再由下一次 Esc 关闭历史窗口。
- 历史窗口打开后输入可打印字符会自动展开搜索并写入搜索内容；搜索框聚焦后，后续文本输入由原生 `TextField` 处理，事件路由不再拦截普通字符。
- 搜索 Esc 状态机：搜索框有内容时先清空内容，搜索框为空但仍展开时关闭搜索框，搜索框未展开时关闭历史窗口；搜索框清空按钮只清空内容。
- 输入即搜索展开后，搜索框光标必须停在文本末尾，不选中已有输入；搜索框聚焦时 Esc 仍走轻贴搜索状态机，Delete 和 `Command + A` 保持原生文本编辑行为。
- 搜索框展开后必须显式成为第一响应者并显示光标；面板级 Esc 也必须进入同一搜索状态机，不能直接关闭轻贴。
- 搜索框聚焦时，除 Esc 和 `Command + 1-9` 外，其他快捷键交给原生文本框；Esc 按清空内容、关闭搜索框、关闭轻贴执行；`Command + 1-9` 直接粘贴对应结果卡片。搜索框展开但未聚焦时，Esc 先清空搜索内容并关闭搜索框，再由下一次 Esc 关闭主窗口。主窗口显示期间点击非主窗口位置会关闭轻贴。
- 搜索必须避免主线程高频全量计算：历史窗口维护预览项和过滤结果缓存，搜索输入使用可取消防抖任务，过滤在后台线程执行，主线程只应用最新结果；图片和 App 图标预热必须延迟执行。
- 搜索性能优化不能牺牲结果完整性；历史窗口必须保留完整搜索结果。搜索框在非激活窗口中使用自定义原生文本框处理 `Command + A/C/X/V/Z/W`，搜索结果更新时禁用隐式动画，避免输入期间大量卡片刷新造成掉帧。
- 历史窗口从 Store 构建预览项和搜索索引时必须走后台任务，主线程只应用生成后的完整数组，避免 1,000/10,000 条历史刷新时阻塞界面。
- Store 不再使用固定 80 条内存上限裁剪历史；保存期限策略负责清理普通历史，置顶内容不自动清理。开发阶段提供批量生成测试文本入口，用于验证 1,000 到 10,000 条规模下的搜索和渲染表现。
- Store 对普通新增、导入和置顶变更使用防抖后台写盘，减少连续剪贴板记录和批量测试数据带来的主线程写盘压力；删除、清空、保存期限清理和 App 退出前刷新仍立即落盘，避免历史索引与附件文件不一致。
- 开发测试文本带有固定前缀和轻贴来源标记，更多菜单支持一键清理测试文本，避免性能测试数据长期污染真实历史。
- 开发测试文本生成必须在后台任务完成，主线程只在生成结束后合并一次结果；菜单显示当前测试文本数量，方便验证清理是否彻底。
...
- 备份包导入前必须校验 `history.json` 是否存在；导入后需要显示导入条数、跳过条数和缺失附件数量。
- 设置页导入、导出和备份包文件读写必须在后台任务执行，操作期间禁用对应按钮，并显示正在处理状态，避免大历史或大附件备份时卡住设置窗口。
- 设置页导入、导出和备份包操作失败时必须显示具体错误原因，并写入系统日志，避免只显示“失败”导致无法定位问题。
- 设置页可清理孤立附件：扫描 `Images`、`Thumbnails` 和 `RichTexts`，删除没有被当前历史记录引用的文件，并刷新存储用量；存储用量计算和孤立附件清理必须在后台任务执行，清理前需要确认，避免设置窗口卡顿或误删。
- 设置页可执行只读数据健康检查：统计缺失图片、缺失富文本、孤立附件数量和孤立附件占用空间，不自动修改任何文件；检查完成后用系统对话框展示详情。
...
- 粘贴执行链路必须返回明确结果：已粘贴、仅复制、失败原因。
- 图片文件缺失、图片无法读取或剪贴板写入失败时必须给出用户可见提示，不能静默失败。
- 历史窗口必须以非激活面板显示，不激活轻贴、不抢走原 App 焦点。自动粘贴前立即隐藏历史窗口，再发送系统级 `Command + V`，让粘贴落到原本聚焦的 App。
...
- 主窗口打开阶段只渲染首屏附近的固定数量卡片，窗口动画完成后恢复完整列表。
- 主窗口打开和筛选变化时预热前几张图片缩略图与来源 App 图标到内存缓存。

...
轻贴 ClipEase 第二版已正式切到 `2.x` 版本线。此前 V2 开发和验收包曾沿用 `1.0.x` patch 线推进，现已按项目规则完成大版本开发 `+1` 收口：`2.0.0(260519.2258)` 为 V2 大版本切线包；当前稳定性收口包为 `2.0.47(260521.0622)`。
...
- `scripts/smoke_check.py`：待本次 `2.0.47(260521.0622)` 文档对齐后重跑
...
- `scripts/build-app.sh --bump patch --run`：通过；当前运行包为 `2.0.47 (260521.0622)`，App bundle 启动方式已由 `scripts/build-app.sh --run` 固化，PID `9374`
- GitHub 推送：通过
...
- 当前候选版本：`2.0.47`
- 当前构建号：`260521.0622`
- 当前 build/run：`2.0.47 (260521.0622)`
- 当前运行进程：PID `9374`
...
- `2.0.47(260521.0622)`：Office / 表格类文件预览稳定性收口包；当前结论为保留格式优先，`doc/docx/xls/xlsx/csv/ppt/pptx/rtf/numbers/pages/key` 等走嵌入式 Quick Look，普通文本文件继续走可选择文本预览；移除 Office 纯文本预览和底部可复制文本区方案；确认公开 `QLPreviewView` 无法稳定强制等同 Finder 空格预览的 Office 内容选择 / 复制能力，私有 Quick Look 激活接口禁止使用。新增当前稳定性验收清单和已知限制记录，后续由用户人工验收。

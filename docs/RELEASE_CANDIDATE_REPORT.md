...
轻贴 ClipEase 第二版已正式切到 `2.x` 版本线。此前 V2 开发和验收包曾沿用 `1.0.x` patch 线推进，现已按项目规则完成大版本开发 `+1` 收口：`2.0.0(260519.2258)` 为 V2 大版本切线包；当前稳定性收口包为 `2.0.51(260521.1641)`。
...
- `scripts/smoke_check.py`：通过，已对齐 `2.0.51(260521.1641)`
...
- `scripts/build-app.sh --bump patch --run`：通过；当前运行包为 `2.0.51 (260521.1641)`，App bundle 启动方式已由 `scripts/build-app.sh --run` 固化，PID `78786`
- GitHub 推送：通过
...
- 当前候选版本：`2.0.51`
- 当前构建号：`260521.1641`
- 当前 build/run：`2.0.51 (260521.1641)`
- 当前运行进程：PID `78786`
